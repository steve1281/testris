...
from rgb import RED, DARK_SLATE_GRAY, ROYAL_BLUE, ORANGE, CYAN, YELLOW, MAGENTA, GREEN, BLACK
...
def is_collision(n_x, n_y):
    check_board = board[n_x:n_x + m.shape[0], n_y:n_y + m.shape[1]]
    return np.size(np.nonzero(np.logical_and(m, check_board)))
    ...
    for event in pygame.event.get():
        ...
        if event.type == pygame.KEYDOWN:
            ...
            if event.key == pygame.K_LEFT:
                if not is_collision(m_x, m_y - 1):
                    m_y = m_y - 1
            if event.key == pygame.K_UP:
                m = np.rot90(m)
                m = np.rot90(m)
                m = np.rot90(m)
                if is_collision(m_x, m_y):
                    m = np.rot90(m)
